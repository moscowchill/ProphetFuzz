...
sudo python3 $manpage_parser_dir/parser.py --file $manpage_parser_dir/input/${program}.1

echo "[*] Restructuring the man page ..."
sudo python3 $llm_interface_dir/restruct_manpage.py --file $manpage_parser_dir/output/manpage_${program}.json

echo "[*] Extracting constraints ..."
sudo python3 $llm_interface_dir/constraint.py --file $llm_interface_dir/input/manpage_${program}.json

echo "[*] Predicting High-Risk Option Combinations ..."
echo $llm_interface_dir
sudo python3 $llm_interface_dir/predict.py --file $llm_interface_dir/input/manpage_${program}.json

echo "[*] Assembling the commands ..."
sudo python3 $llm_interface_dir/assemble.py --file $llm_interface_dir/input/manpage_${program}.json

echo "[*] Generating files from the code ..."
sudo python3 $fuzzing_handler_dir/generate_file.py

echo "[*] Generating Argvs for fuzzing ..."
sudo python3 $fuzzing_handler_dir/fix_argvs.py

echo "[*] Running corpus minimization ..."
sudo python3 $fuzzing_handler_dir/run_cmin.py --program $program
